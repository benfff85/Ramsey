...
public class GraphMutator {
	 ...
	 * 
	 * @param cayleyGraph
	 *            The CayleyGraph object to be mutated.
	 */
	...
	/**
	 * This will return the MUTATION_TYPE for this particular mutation. This allows both primary and secondary mutation types to be leveraged.
	 * 
	...
	/**
	 * This will call the appropriate mutation method based on which MUTATION_TYPE is needed for this particular mutation.
	 * 
	 * @param mutateType
	 *            The type of mutation to be used this mutation.
	 * @return void
	 */
	private void mutateGraph(MUTATION_TYPE mutateType) {
		if (mutateType == MUTATION_TYPE.RANDOM) {
			...
			if (mutateType == MUTATION_TYPE.TARGETED) {
				mutateGraphTargeted();
			} else
				if (mutateType == MUTATION_TYPE.BALANCED) {
	...
	/**
	 * This will flip the color of one edge from the identified clique along with one random edge of the opposite color to maintain balance.
	 * 
	 * @return void
	 */
	...
	/*
	 * private void mutateGraphBalancedAll(){ for (int i = 0; i < cayleyGraph.getCliqueCollection().getCliqueCount(); i++){ if (cayleyGraph.getCliqueCollection().getCliqueByIndex(i).validateClique()){ mutateGraphBalanced(cayleyGraph.getCliqueCollection().getCliqueByIndex(i)); } } }
	 */
	...
	/**
	 * This will flip the color of one edge from the identified clique along with one other edge of opposite color logically selected based off of color distribution.
	 * 
	 * @return void
	 */
	...
	private Edge getCliqueEdgeOfHighestRank(Clique clique) {
		...
		for (int i = 0; i < clique.getCliqueSize(); i++) {
			if (clique.getCliqueVertexByPosition(i).getEdgeCount(cliqueColor) > connectedEdgeCountA) {
				...
				if (clique.getCliqueVertexByPosition(i).getEdgeCount(cliqueColor) > connectedEdgeCountB) {
					vertexListB.clear();
	...
	/**
	 * This function uses a random number generator to return a random vertex from an input Vertex ArrayList.
	 * 
	 * @param vertexList
	 *            Vertex ArrayList from which a vertex will be returned at random.
