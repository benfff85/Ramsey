...
/**
 * This is the main Controller Class where the program will initiate. It handles the outermost loop of logic
 * which encompasses a framework for the entire application. 
 * 
...
public class Controller {
	...
	/**
	 * This is the main method for the application. It will initialize the CayleyGraph object and begin searching for a 
	 * counter example. A counter example is defined as a CayleyGraph with no clique (complete subgraph) within it.
	 * This method will loop continually searching and mutating until a counter example is found.
	 * <p>
	 * It is noted the passing of Strings to methods as done below for the purposes of switch statements is generally poor
	 * practice, however it has done here to allow for quick modifications and testing of the code without the use of a GUI.
	 * 
	 * @param args
	 * @return void
	 */
	public static void main (String [] args) {
		...
		while(!counterExampleFound){
			...
			if(logger.getAnalyzedGraphCount()%1000==0){
				printNegativeCase(cayleyGraph, logger, timer, "CONSOLE");
	...
	/**
	 * This method is responsible for rotating the CayleyGraphs vertices to prevent detection of cliques in the low
	 * vertices from causing issues with edge distribution.
	 * 
	...
	/**
	 * This method will dictate the method of mutation used on the CayleyGraph after a counter example is found
	 * 
	...
	/**
	 * Sets the size of the CayleyGraph for which we will be searching for a counter example from several common sizes
	 * 
	 * @param size This is the size of the test being run which is selected from a predefined list of common sizes. Valid values are:<br>
	...
	/**
	 * Outputs statistics in the event of a counter example not being found to the console,
	 * a log file, or both.
	 * 
	 * 
	 ...
	 * @param outputMethod A String signifying the preferred output method. Valid values are "CONSOLE", "FILE", "BOTH"
	 * @return void
	 */
	...
	/**
	 * Outputs the counter example in the event it has been found.
	 * This will write to the log file, email, print to the console and write to a solution file Ramsey.sol.
	 * All the above will be in a format compatible with Mathematica with the exception of the .sol file. The .sol
	 * file will be comma separated digits so that it may be used as input to the CayleyGraph.loadFromFile method later.
	 * 
